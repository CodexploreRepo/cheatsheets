...
| `eval `           | used to execute arguments as a shell command.<br>Example 1: `cd_var ="cd Desktop"; eval $cd_var` this `eval` function will execute the content inside the `cd_var`<br>Example 2: `eval $(ssh-agent -s)` you can directly call the function inside the `eval $()` if you want the process created from that function is perfoming in the same current shell rather than starting a new sub-shell |
